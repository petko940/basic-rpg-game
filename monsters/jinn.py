...
class Jinn(Monster):
    ...
    def __init__(self, health: int, damage: int, x_pos: int, y_pos: int):
        ...
        self.attack_right = [transform.scale(image.load(f'monsters/jinn_sprites/attack/{i}.png'), (310, 314)) for i in range(4)]
        self.attack_left = [transform.flip(self.attack_right[i], True, False) for i in range(len(self.attack_right))]

        self.die_right = [transform.scale(image.load(f'monsters/jinn_sprites/death/{i}.png'), (310, 314)) for i in range(6)]
        self.die_left = [transform.flip(self.die_right[i], True, False) for i in range(len(self.die_right))]

        self.idle_right = [transform.scale(image.load(f'monsters/jinn_sprites/idle/{i}.png'), (310, 314)) for i in range(3)]
        self.idle_left = [transform.flip(self.idle_right[i], True, False) for i in range(len(self.idle_right))]

        self.walk_right = [transform.scale(image.load(f'monsters/jinn_sprites/walk/{i}.png'), (310, 314)) for i in range(4)]
    ...
    def check_end_of_attack(self):
        if int(self.non_looped_index) >= len(self.attack_left):
            ...
            self.attack_cooldown = Jinn.ATTACK_COOLDOWN
            self.non_looped_index = 0

    def check_target_reached(self, hero):
        difference_left = abs((self.x_pos - Jinn.LOWER_HIT_BOX) - hero.x)
        difference_right = abs((self.x_pos + Jinn.LOWER_HIT_BOX) - hero.x)

        if difference_left <= 70 or difference_right <= 70 or difference_left + difference_right == Jinn.LOWER_HIT_BOX * 2:
            return True
        return False

    def walk(self, hero_x_pos: int):
        self.walk_and_idle_index += Jinn.IMAGE_LOOP_SPEED

        image_index = int(self.walk_and_idle_index) % len(self.walk_left)

        if self.x_pos > hero_x_pos:
            self.x_pos -= Jinn.MOVE_SPEED
            self.health_bar.x -= Jinn.MOVE_SPEED
            self.background_health_bar.x -= Jinn.MOVE_SPEED
    ...
    def increase_death_image_index(self):
        self.non_looped_index += Jinn.IMAGE_LOOP_SPEED
    ...
    def health_bar_position(self):
        ...
        return self.health_bar.x + 30, self.y_pos + 15, self.health_bar.width, self.HEALTH_BAR_HEIGHT
    ...
    def health_bar_pad_position(self):
        ...
        return self.background_health_bar.x + 30, self.y_pos + 15, self.background_health_bar.width, self.HEALTH_BAR_HEIGHT